...
public class UnitTestUsuarioController {
	...
	void init() {
		...
		BDDMockito.given(manager.loadUserByUsername(login))
				.willReturn(new UsuarioDTODetails(new UsuarioDTO(login, "1234")));
	}

	@Test
	void testLoginPage() throws Exception {
		mvc.perform(get(rootPath + "/login")).andExpect(status().isOk());
	}

	@Test
	void testLogeandose() throws Exception {

		mvc.perform(post(rootPath + "/login").param("username", "admin").param("password", "secreto").with(csrf()))
				.andExpect(ResultMatcher.matchAll(status().is3xxRedirection(), redirectedUrl("/")));
	}

	@Test
	void testRegistrandose() throws Exception {
		mvc.perform(post(rootPath + "/registro").contentType(MediaType.APPLICATION_FORM_URLENCODED)
				.param("login", "flo00008").param("email", "flo00008@red.ujaen.es")
				.param("password", "6~\\n=X}Q+tTR5fW!").param("terminos", "false")// TODO no debería pasar el test sin
																					// aceptar los terminos
				.with(csrf())).andExpect(ResultMatcher.matchAll(status().is3xxRedirection(), redirectedUrl("/")));
	}

	@Test
	void testErrorRegistro() throws Exception {
		mvc.perform(post(rootPath + "/registro").contentType(MediaType.APPLICATION_FORM_URLENCODED)
				.param("login", "flo00008").param("email", "flo00008@red.ujaen.es").param("password", "X")// Cotnraseña
																											// poco
																											// segura
	...
	void testRegsitroPage() throws Exception {
		mvc.perform(get(rootPath + "/registro")).andExpect(status().isOk());
	}

	@Test
	void testPermisosPerfil() throws Exception {
		mvc.perform(get(rootPath + "/perfil")).andExpect(
				ResultMatcher.matchAll(status().is3xxRedirection(), redirectedUrlPattern("**" + rootPath + "/login")));
	}
	...
	void testAccesoPerfil() throws Exception {
		mvc.perform(get(rootPath + "/perfil")).andExpect(
				ResultMatcher.matchAll(status().isOk(), model().attributeExists("usuario", "creados", "inscritos")));
	}
