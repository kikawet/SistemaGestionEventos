...
public class TestRest {
	...
	void registroYLoginUsuario() throws Exception {
		...
		// registro
		mvc.perform(post(rootPath + "/usuario/registro").contentType(MediaType.APPLICATION_JSON)
				.content(jsonObjectMapper.writeValueAsString(usuarioDTO)).accept(MediaType.APPLICATION_JSON))
		...
		mvc.perform(post(rootPath + "/usuario/login", UUIDUsuario).contentType(MediaType.APPLICATION_JSON)
				.content(jsonObjectMapper.writeValueAsString(usuarioDTO)).accept(MediaType.APPLICATION_JSON))
	...
	void listarInscritosTest() throws Exception {
		...
		mvc.perform(get(rootPath + "/usuario/{uId}/inscritos", UUIDusuario).param("uId", UUIDusuario)
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(ResultMatcher.matchAll(status().isOk(), content().contentType(MediaType.APPLICATION_JSON),
						jsonPath(".eventoDTOList").isArray(), jsonPath(".anterior").doesNotExist(),
		...
		mvc.perform(get(rootPath + "/usuario/{uId}/inscritos", UUIDusuario).param("uId", UUIDusuario).param("cant", "1")
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(ResultMatcher.matchAll(status().isOk(), content().contentType(MediaType.APPLICATION_JSON),
						jsonPath(".eventoDTOList.length()").value(1), jsonPath(".anterior").doesNotExist(),
		...
		mvc.perform(get(rootPath + "/usuario/{uId}/inscritos", UUIDusuario).param("uId", UUIDusuario).param("page", "1")
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(ResultMatcher.matchAll(status().isOk(), content().contentType(MediaType.APPLICATION_JSON),
						jsonPath(".anterior").exists(), jsonPath(".siguiente").doesNotExist()));
